...
clip_logged_flags = defaultdict(bool)
...
def compute_velocity_log(cam, mode, start, end):
    ...
    z_dir = "북" if v[2] > 0 else "남"
    y_dir = "상승중" if v[1] > 0 else "하강중"

    print(f"[🧮 속도 계산] Δx={delta[0]:.2f}, Δy={delta[1]:.2f}, Δz={delta[2]:.2f} | duration={duration:.2f}")

    return f"{x_dir}쪽으로 {abs(v[0]):.2f}m/s, {z_dir}쪽으로 {abs(v[2]):.2f}m/s, {abs(v[1]):.2f}m/s 속력으로 {y_dir}"



def save_clip_buffer(cam, mode, label, buffer, fps, size, avg_conf=None, cluster_id=None):
    ...
    os.system(
        ...
        f"-c:v libx264 -preset fast -crf 23 -movflags +faststart {final_path}"
    )
    os.remove(temp_path)
    ...
    # ✅ 중복 로그 방지
    clip_key = f"{cam}_{mode}_{label}_{frame_start}_{frame_end}"
    if cluster_id is not None:
        clip_key = f"{cam}_{mode}_{label}_{cluster_id}"
    if clip_logged_flags.get(clip_key, False):
        print(f"[⚠️ 중복 로그 방지됨] {clip_key}")
        return
    clip_logged_flags[clip_key] = True
    ...
    if log_callback_fn:
        ...
        print(f"[DEBUG] Calling log_callback with: {log_cam}, {label_str}, {final_path}, {message}")
    ...
    try:
        ...
        if not velocity_flags.get(cluster_key, False) and mode != "thermal" and log_callback_fn:
            # velocity에도 cam1_rgb 형태로 전달하도록 변경
            log_callback_fn(f"{cam}_{mode}", "velocity", velocity_log)
    ...
    while True:
        ...
        for start, end, cluster_label, avg_conf in record_tasks[task_key]:
            ...
            if start <= frame_idx <= end:
                ...
                if clip_buffer:
                    save_clip_buffer(cam, mode, cluster_label, clip_buffer, fps, (width, height), avg_conf=avg_conf, cluster_id=f"{start}_{end}")
